...
function handleMarkers(data, map) {
    ...
    for (var i = 0; i < data.base.length; i++) {
        ...
        var marker2 = new L.Marker([data.base[i].Latitude, data.base[i].Longitude], {draggable: true, icon: customIcon3 }).addTo(map);
    ...
    var prev_coordinates = marker2.getLatLng();
    marker2.on('drag', function (event) {
        kuklos2.setLatLng(event.target.getLatLng());
    });
    ...
    document.getElementById('confirmMove').addEventListener('click', function () {
        make_ajax_post(marker2);
        prev_coordinates = marker2.getLatLng();
        document.getElementById('confirmationModal').style.display = 'none';
    });
    //Diaxeirisi koumpioy cancel
    document.getElementById('cancelMove').addEventListener('click', function () {
        document.getElementById('confirmationModal').style.display = 'none';
        marker2.setLatLng( prev_coordinates);
        kuklos2.setLatLng( prev_coordinates);
    });
    //Dimiourgia pop up
    var popup = L.popup();
